...
<p>The air intake moves at 100 m/s; correspondingly, the pressure at the outlet is increased by about 6000 Pa and the temperature by about 5 K with respect to the environment values.</p>
    ...
    model TestFan "Test bench of Fan model"
      ...
      Modelica.Units.SI.Pressure P_E=GSP_c.P_E_nom*(1 + time/10)
        "Inlet pressures are free to change";
...
<p>The combustion chamber starts at T = 500 degC, then it reaches the equilibrium temperature of 996 degC in about 0.1 seconds.</p>
    ...
    model TestSampleCompressorReplaceable "Compressor test bench with replaceable CompressorData dataset, defaults to SampleCompressor"
      ...
      parameter Modelica.Units.SI.PerUnit N_n_min=compressorData.Phi_n_table[2,
          1] "Minimum value of N_n in the data record";
      parameter Modelica.Units.SI.PerUnit N_n_max=compressorData.Phi_n_table[
          end, 1] "Minimum value of N_n in the data record";
      parameter Integer N = 2*Nv - 1 "Number of test cases, one for each N_n value plus one for each intermediate value";
      parameter Modelica.Units.SI.PerUnit N_n[N]=linspace(
              N_n_min,
              N_n_max,
              N) "Angular velocity will be fixed for each test instance";
  ...
  annotation(experiment(StopTime = 1, Interval = 0.01));
    ...
    model Test_GSP_FanCore_Compressor "Compressor test bench with GSP_FanCore dataset"
      extends BasicAeroEngines.Tests.TestComponents.TestSampleCompressorReplaceable(redeclare
          record                                                                                     CompressorData =
            BasicAeroEngines.Data.Compressors.GSP_FanCore);
      annotation (
...
<p>To check the model, you can use 2D plots with <code>Pcompressors[j].Phi_n</code>on the abscissa and <code>Pcompressors[j].PR_n</code>, one plot for each j corresponding to a specific value of the reduced speed. You can also plot the values of <code>eta</code> against, e.g., <code>beta</code> values.</p>
    ...
    model Test_GSP_HPC_Compressor "Compressor test bench with GSP_HPC dataset"
      extends BasicAeroEngines.Tests.TestComponents.TestSampleCompressorReplaceable(redeclare
          record                                                                                     CompressorData =
            BasicAeroEngines.Data.Compressors.GSP_HPC,                                                                                                           compressors(each useHomotopy = true));
      annotation (
        experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 0.002),
  ...
  <p>To check the model, you can use 2D plots with <code>Pcompressors[j].Phi_n</code>on the abscissa and <code>Pcompressors[j].PR_n</code>, one plot for each j corresponding to a specific value of the reduced speed. You can also plot the values of <code>eta</code> against, e.g., <code>beta</code> values.</p>
    ...
    model Test_GSP_LPC_Compressor "Compressor test bench with GSP_LPC dataset"
      extends BasicAeroEngines.Tests.TestComponents.TestSampleCompressorReplaceable(redeclare
          record                                                                                     CompressorData =
            BasicAeroEngines.Data.Compressors.GSP_LPC);
      annotation (
        experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 0.002),
  ...
  <p>To check the model, you can use 2D plots with <code>Pcompressors[j].Phi_n</code>on the abscissa and <code>Pcompressors[j].PR_n</code>, one plot for each j corresponding to a specific value of the reduced speed. You can also plot the values of <code>eta</code> against, e.g., <code>beta</code> values.</p>
    ...
    model TestSampleTurbine "Turbine test bench with SampleTurbine data"
      ...
      parameter Modelica.Units.SI.PerUnit N_n_min=turbineData.eta_n_table[2, 1]
        "Minimum value of N_n in the data record";
      parameter Modelica.Units.SI.PerUnit N_n_max=turbineData.eta_n_table[end,
          1] "Minimum value of N_n in the data record";
      parameter Modelica.Units.SI.PerUnit Phi_n_min=0.4
        "Minimum value of Phi_n for the experiment";
      parameter Modelica.Units.SI.PerUnit Phi_n_max=1.0
        "Minimum value of Phi_n for the experiment";
      parameter Integer N = 2*Nv - 1 "Number of test cases, one for each N_n value plus one for each intermediate value";
      parameter Modelica.Units.SI.PerUnit N_n[N]=linspace(
              N_n_min,
...
<p>The simulation starts near steady state on the ground at zero airspeed, with a fuel flow of 0.279 kg/s. At time 10, the fuel flow is increased with a two-second ramp to 0.35 kg/s.</p>
    ...
    model CooledTurbineTest
      ...
      BasicAeroEngines.Components.CooledTurbine HPT(CoolingTechRot = {2}, CoolingTechStat = {2}, Nbleed = 1, Nstages = 1, Nstages_Bleeds = {1}, Tblade(displayUnit = "K"), Xi_cool_rot = {0.048*w25/fnom}, Xi_cool_stat = {0.05*w25/fnom}, data = HPTdata, PRnom = 2, P_L_iso_start = Phpt_L.*ones(HPT.Nstages), f_E(start = 63.48),
        redeclare
          BasicAeroEngines.Components.TurbineCoolingModel.FixedCoolingFraction
          StatorCooling[HPT.Nstages](coolingFraction=HPT.Xi_cool_stat),
        redeclare
          BasicAeroEngines.Components.TurbineCoolingModel.FixedCoolingFraction
      ...
      BasicAeroEngines.Components.CooledTurbine LPT(                      CoolingTechRot = {1, 1}, CoolingTechStat = {2, 1}, Nbleed = 1, Nstages = 2, Nstages_Bleeds = {2}, PRnom = 1.39, Tblade(displayUnit = "K"), Xi_cool_rot = {0.0, 0.0}, Xi_cool_stat = {0.017*w25/fnom, 0.0}, data = LPTdata,
        redeclare
          BasicAeroEngines.Components.TurbineCoolingModel.FixedCoolingFraction
          StatorCooling[LPT.Nstages](coolingFraction=LPT.Xi_cool_stat),
        redeclare
          BasicAeroEngines.Components.TurbineCoolingModel.FixedCoolingFraction
